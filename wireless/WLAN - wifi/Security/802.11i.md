...
- authentication can be based on strong protocol ([[cybersec/defence/TLS]])
...
PMK can be:
- preshared (WPA-personal) -> PSK
- negotiated (WPA-enterprise) -> MSK
...
PMK
- In **WPA/WPA2-Enterprise**, **PMK is derived from the MSK** after EAP authentication.
- In **WPA/WPA2-Personal**, **PMK is derived from the PSK** using PBKDF2.
...
The message integrity check (MIC), per-packet key hashing, broadcast key rotation, and a sequence counter (IV) discourage many attacks

TKIP is vulnerable to **MIC key recover**
Similar to chop chop 
