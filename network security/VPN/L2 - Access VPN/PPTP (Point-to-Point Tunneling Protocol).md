...
PPTP uses the MPPE algorithm for encryption and the MS-CHAP for authentication (both created/modified by Microsoft). Additionally, it uses the PNS (PPTP Network Server), which is the corporate (VPN) gateway, and the PAC (PPTP Access Concentrator), used only in provider-provisioned deployment mode. Finally, it employs double tunneling (PPP tunneling and then [[network security/VPN/L3 - IPsec/GRE - Generic Routing Encapsulation|GRE]] for tunneling of PPP over IP) for the PPTP Data Tunneling and requires a Control Connection that uses TCP encapsulation for managing the data tunnel setup, management, and teardown.